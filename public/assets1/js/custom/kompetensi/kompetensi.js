...
/*
|-------------------------------------------------------------------
| MODUL KOMPETENSI AJAX + SWEETALERT2 + DATATABLES
|-------------------------------------------------------------------
| Script ini menangani interaksi halaman master kompetensi keahlian
...
var KTKompetensi = (function () {
    ...
    var kompetensiConfig = window.kompetensiConfig || {};
    var baseUrl = kompetensiConfig.baseUrl
        ? kompetensiConfig.baseUrl
        : window.location.origin;
    var urlSimpan = kompetensiConfig.urlSimpan || (baseUrl.replace(/\/$/, "") + "/superadmin/kompetensi/simpan");
    var urlUpdate = kompetensiConfig.urlUpdate || (baseUrl.replace(/\/$/, "") + "/superadmin/kompetensi/update");
    var urlHapus = kompetensiConfig.urlHapus || (baseUrl.replace(/\/$/, "") + "/superadmin/kompetensi/hapus");
    ...
    */
    ...
    | Alur kerja: plugin FormValidation diinisialisasi untuk 2 field utama.
    |
    | Tips Debugging:
            ...
            fields: {
                nama_kompetensi: {
                    validators: {
                        ...
                        stringLength: {
                            max: 100,
                            message: "Nama kompetensi maksimal 100 karakter."
                        }
                    }
                },
                akronim: {
                    validators: {
                        notEmpty: {
                            message: "Akronim wajib diisi."
                        },
                        stringLength: {
                            max: 20,
                            message: "Akronim maksimal 20 karakter."
                        }
                    }
                }
            },
    ...
    */
    ...
    | data attribute baris kompetensi yang dipilih.
    | Alur kerja: dataset row dibaca lalu dimasukkan ke input form edit.
    |
    | Tips Debugging:
    | - Jika field kosong, periksa data-id, data-nama, dan data-akronim.
    */
    ...
    |-------------------------------------------------------------------
    | Menjadikan tabel kompetensi dapat dicari dan diurutkan secara
    | client-side dengan pengecualian kolom checkbox dan aksi.
    | Alur kerja: DataTables dipasang lalu input search dihubungkan.
    |
    | Tips Debugging:
    | - Jika tabel tidak berubah, periksa selector #kt_kompetensi_table.
    */
    var initDataTable = function () {
        tableElement = document.querySelector("#kt_kompetensi_table");
        searchInput = document.querySelector('[data-kt-kompetensi-filter="search"]');
    ...
    */
    var initAddModal = function () {
        addModalElement = document.getElementById("kt_modal_tambah_kompetensi");
        addForm = document.getElementById("kt_modal_tambah_kompetensi_form");
        ...
        addModalElement.querySelector('[data-kt-kompetensi-modal-action="close"]').addEventListener("click", function (event) {
            handleCloseAction(event, addModal);
        });

        addModalElement.querySelector('[data-kt-kompetensi-modal-action="cancel"]').addEventListener("click", function (event) {
        ...
        addForm.addEventListener("submit", function (event) {
            ...
            validateForm(addValidator, addForm).then(function (status) {
                if (status !== "Valid") {
                    showErrorAlert("Form kompetensi masih belum valid. Silakan periksa kembali.");
                    return;
                }
                ...
                requestJson(
                    ...
                    .then(function (responseData) {
                        ...
                        addModal.hide();
                        showSuccessAndReload(responseData.message || "Data kompetensi berhasil disimpan.");
                    })
                    .catch(function (error) {
    ...
    */
    var initEditModal = function () {
        editModalElement = document.getElementById("kt_modal_edit_kompetensi");
        editForm = document.getElementById("kt_modal_edit_kompetensi_form");
        ...
        editModalElement.querySelector('[data-kt-kompetensi-edit-modal-action="close"]').addEventListener("click", function (event) {
            handleCloseAction(event, editModal);
        });

        editModalElement.querySelector('[data-kt-kompetensi-edit-modal-action="cancel"]').addEventListener("click", function (event) {
        ...
        editForm.addEventListener("submit", function (event) {
            ...
            if (!idKompetensi) {
                showErrorAlert("Data kompetensi yang akan diedit tidak valid.");
                return;
            }
            ...
            validateForm(editValidator, editForm).then(function (status) {
                ...
                requestJson(
                    urlUpdate.replace(/\/$/, "") + "/" + idKompetensi,
                    "POST",
                    ...
                    .then(function (responseData) {
                        ...
                        editModal.hide();
                        showSuccessAndReload(responseData.message || "Data kompetensi berhasil diperbarui.");
                    })
                    .catch(function (error) {
                        showErrorAlert(error.message);
                    })
    ...
    |
    | Tips Debugging:
    | - Jika tombol tidak aktif, periksa data-kt-kompetensi-table-filter.
    */
    var initTableActions = function () {
        ...
        tableElement.addEventListener("click", function (event) {
            var editButton = event.target.closest('[data-kt-kompetensi-table-filter="edit_row"]');
            var deleteButton = event.target.closest('[data-kt-kompetensi-table-filter="delete_row"]');
    ...
    */
    var handleDeleteRow = function (rowElement) {
        ...
        var namaKompetensi = rowElement.getAttribute("data-nama") || "data ini";

        if (!idKompetensi) {
            showErrorAlert("ID kompetensi tidak ditemukan.");
            return;
        }

        Swal.fire({
            text: "Apakah Anda yakin ingin menghapus " + namaKompetensi + "?",
            icon: "warning",
            showCancelButton: true,
            buttonsStyling: false,
            ...
            if (!result.isConfirmed) {
                if (result.dismiss === Swal.DismissReason.cancel) {
                    Swal.fire({
                        text: namaKompetensi + " tidak dihapus.",
                        icon: "info",
                        buttonsStyling: false,
                        confirmButtonText: "Oke",
                        customClass: {
            ...
            requestJson(
                urlHapus.replace(/\/$/, "") + "/" + idKompetensi,
                "GET"
            )
                .then(function (responseData) {
                    Swal.fire({
                        text: responseData.message || "Data kompetensi berhasil dihapus.",
                        icon: "success",
                        buttonsStyling: false,
                        confirmButtonText: "Oke",
                        customClass: {
