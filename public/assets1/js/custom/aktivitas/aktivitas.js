...
/*
|-------------------------------------------------------------------
| MODUL AKTIVITAS AJAX + SWEETALERT2 + DATATABLES
|-------------------------------------------------------------------
| Script ini menangani interaksi halaman master aktivitas alumni
...
var KTAktivitas = (function () {
    ...
    var aktivitasConfig = window.aktivitasConfig || {};
    var baseUrl = aktivitasConfig.baseUrl
        ? aktivitasConfig.baseUrl
        : window.location.origin;
    var urlSimpan = aktivitasConfig.urlSimpan || (baseUrl.replace(/\/$/, "") + "/superadmin/aktivitas/simpan");
    var urlUpdate = aktivitasConfig.urlUpdate || (baseUrl.replace(/\/$/, "") + "/superadmin/aktivitas/update");
    var urlHapus = aktivitasConfig.urlHapus || (baseUrl.replace(/\/$/, "") + "/superadmin/aktivitas/hapus");
    ...
    */
    ...
    | Alur kerja: plugin FormValidation diinisialisasi untuk field aktivitas.
    |
    | Tips Debugging:
            ...
            fields: {
                nama_aktivitas: {
                    validators: {
                        ...
                        stringLength: {
                            max: 100,
                            message: "Nama aktivitas maksimal 100 karakter."
                        }
                    }
                }
            },
    ...
    */
    ...
    | data attribute baris aktivitas yang dipilih.
    | Alur kerja: dataset row dibaca lalu dimasukkan ke input form edit.
    |
    | Tips Debugging:
    | - Jika field kosong, periksa data-id, data-nama, dan data-keterangan.
    */
    ...
    |-------------------------------------------------------------------
    | Menjadikan tabel aktivitas dapat dicari dan diurutkan secara
    | client-side dengan pengecualian kolom checkbox dan aksi.
    | Alur kerja: DataTables dipasang lalu input search dihubungkan.
    |
    | Tips Debugging:
    | - Jika tabel tidak berubah, periksa selector #kt_aktivitas_table.
    */
    var initDataTable = function () {
        tableElement = document.querySelector("#kt_aktivitas_table");
        searchInput = document.querySelector('[data-kt-aktivitas-filter="search"]');
    ...
    */
    var initAddModal = function () {
        addModalElement = document.getElementById("kt_modal_tambah_aktivitas");
        addForm = document.getElementById("kt_modal_tambah_aktivitas_form");
        ...
        addModalElement.querySelector('[data-kt-aktivitas-modal-action="close"]').addEventListener("click", function (event) {
            handleCloseAction(event, addModal);
        });

        addModalElement.querySelector('[data-kt-aktivitas-modal-action="cancel"]').addEventListener("click", function (event) {
        ...
        addForm.addEventListener("submit", function (event) {
            ...
            validateForm(addValidator, addForm).then(function (status) {
                if (status !== "Valid") {
                    showErrorAlert("Form aktivitas masih belum valid. Silakan periksa kembali.");
                    return;
                }
                ...
                requestJson(
                    ...
                    .then(function (responseData) {
                        ...
                        addModal.hide();
                        showSuccessAndReload(responseData.message || "Data aktivitas berhasil disimpan.");
                    })
                    .catch(function (error) {
    ...
    */
    var initEditModal = function () {
        editModalElement = document.getElementById("kt_modal_edit_aktivitas");
        editForm = document.getElementById("kt_modal_edit_aktivitas_form");
        ...
        editModalElement.querySelector('[data-kt-aktivitas-edit-modal-action="close"]').addEventListener("click", function (event) {
            handleCloseAction(event, editModal);
        });

        editModalElement.querySelector('[data-kt-aktivitas-edit-modal-action="cancel"]').addEventListener("click", function (event) {
        ...
        editForm.addEventListener("submit", function (event) {
            ...
            if (!idAktivitas) {
                showErrorAlert("Data aktivitas yang akan diedit tidak valid.");
                return;
            }
            ...
            validateForm(editValidator, editForm).then(function (status) {
                ...
                requestJson(
                    ...
                    .then(function (responseData) {
                        ...
                        editModal.hide();
                        showSuccessAndReload(responseData.message || "Data aktivitas berhasil diperbarui.");
                    })
                    .catch(function (error) {
                        showErrorAlert(error.message);
                    })
    ...
    |
    | Tips Debugging:
    | - Jika tombol tidak aktif, periksa data-kt-aktivitas-table-filter.
    */
    ...
    */
    var handleDeleteRow = function (rowElement) {
        ...
        var namaAktivitas = rowElement.getAttribute("data-nama") || "data ini";

        if (!idAktivitas) {
            showErrorAlert("ID aktivitas tidak ditemukan.");
            return;
        }

        Swal.fire({
            text: "Apakah Anda yakin ingin menghapus " + namaAktivitas + "?",
            icon: "warning",
            showCancelButton: true,
            buttonsStyling: false,
            ...
            if (!result.isConfirmed) {
                if (result.dismiss === Swal.DismissReason.cancel) {
                    Swal.fire({
                        text: namaAktivitas + " tidak dihapus.",
                        icon: "info",
                        buttonsStyling: false,
                        confirmButtonText: "Oke",
                        customClass: {
            ...
            requestJson(
                urlHapus.replace(/\/$/, "") + "/" + idAktivitas,
                "GET"
            )
                .then(function (responseData) {
                    Swal.fire({
                        text: responseData.message || "Data aktivitas berhasil dihapus.",
                        icon: "success",
                        buttonsStyling: false,
                        confirmButtonText: "Oke",
                        customClass: {
